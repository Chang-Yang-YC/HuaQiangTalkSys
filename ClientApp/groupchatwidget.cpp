    ...
    , ui(new Ui::groupchatWidget)
{
    ui->setupUi(this);
        ...
        //接受信息  此处连接数据库
        connect(timer,&QTimer::timeout,this,&groupchatWidget::on_txt_received);
...
void groupchatWidget::start(int toid, int meid,QString name)
{
    senderId = toid;
    meId = meid;
    friendname = name;
...
void groupchatWidget::on_pushButtonsend_clicked()
{
    //Str = GPTEXT##群ID##DATE##Time##内容
    QDateTime current_date_time =QDateTime::currentDateTime();
    //current_date字符串结果为"2016.05.20 12:17:01.445"
    QString current_date =current_date_time.toString("yyyy.MM.dd");
    QString current_Time = current_date_time.toString("hh:mm:ss.zzz");
    QString head = QString("GPTEXT##%1##%2##%3##").arg(senderId).arg(current_date).arg(current_Time);
    QString str = head +meId+":"+ui->textEditsend->toPlainText();
    tcpClient->write(str);
    ui->textEditsend->clear();
}

void groupchatWidget::on_txt_received()
{
    //id还没传进来
...
void groupchatWidget::on_pushButtonfile_clicked()
{
    fileSendWin.show();
